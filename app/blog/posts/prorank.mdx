...
Last year I did a lot of work with community detection. I read of the applications in protein protein interactions (PPI), but coming from the computing side I didn’t have much context. This post collects together what I learnt from implementing a PPI paper. I picked a paper from 2012 because it was the most recent paper I found that I could slightly understand with such little background knowledge.
...
### The algorithm
The prorank algorithm is split into 5 steps. My python implementation can be found on [GitHub](https://github.com/MattThorne/implementations/tree/master/graphs/prorank).

##### 1. Pruning
First the PPI graph is pruned by weighting the edges and removing edges that fall below a threshold. Each edge on the graph is weighted by comparing the two nodes it connects. The number of neighbours that the 2 nodes share is compared to the number of neighbours the nodes have. This gives an indication of how likely the interaction the edge represents is to exist. For example if 2 proteins interact with all the same other proteins they are likely to interact with each other. Whereas if they don't interact with any common proteins maybe the interaction is a false positive. 
<PPIPruningLow/>
<PPIPruningHigh/>

##### 2. Filtering
Next certain types of proteins in the network are identified based on the topology of the network. These proteins skew the generation of complexes, for example by bridging between 2 complexes that otherwise are unrelated.
<PPIBridge/>

##### 3. Generating a similarity matrix
Next every protein in the network is compared with every other protein. This is done by comparing the protein sequences. It is thought the more similar their sequences are, the more shared evolutionary history there is, and that can act as an indication they should be clustered together.
...
#### Assumptions
- Is this true that proteins that have similar sequences are more likely to be in the same complex?

#### Simple Improvements
- By switching to a modern community detection algorithm the scores were improved by x. 
